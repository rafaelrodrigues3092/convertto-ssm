...
import { openNewFile, validateDocumentType, DocumentStructure, isAutoCopyEnabled, copyToClipboard } from './Util';
      ...
      if (ssmObject) {
        ssmObject['mainSteps'].forEach(function (step) {
          ...
          if (step['action'] === 'aws:runShellScript') {
            ...
            target = 'powershell';
            console.log(step.inputs.runCommand)
            finalScript = step.inputs.runCommand?.join('\r\n');
          }
          else {
          ...
          let outScript: string = finalScript ? finalScript : '';

          if (autoCopy) {
            copyToClipboard(outScript);
          }
          openNewFile(outScript, target);
        });
      }
    }
  }
